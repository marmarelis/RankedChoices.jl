...
## possibly for the future, note that one could use `... where C <: SMatrix{N,N,T}`
## in order to avoid specifying the `L` explicitly, which can't be computed within the type
...
struct VoterMixture{ N, M, T, L } # all locally and statically allocated
  cohorts :: SVector{ M, VoterCohort{ N, T, L } }
